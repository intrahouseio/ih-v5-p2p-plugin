...
const store = { buffer: {}, hwid: null, path: null, sessions: {}  };
...
function messageSocket(socket, sessionid, data) {
  if (data[0] === 0 && data[1] === 255) {
    ...
    if (data[0] === REQ_START) {
      ...
      proxy.proxyData({ port: 3001, syspath: store.path }, socket, session, params);
      ...
      if (store.buffer[session + uuid]) {
        proxy.proxyData({ port: 3001, syspath: store.path }, socket, session, store.buffer[session + uuid].params, Buffer.concat(store.buffer[session + uuid].payload));

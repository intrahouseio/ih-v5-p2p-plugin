...
const store = { buffer: {}, hwid: null, path: null };
...
function messageSocket(socket, sessionid, data) {
  if (data[0] === 0 && data[1] === 255) {
    ...
    if (data[0] === REQ_START) {
      ...
      proxyData({ port: 3001, syspath: store.path }, socket, session, params);
      ...
      if (store.buffer[session + uuid].params.method === 'xhr') {
      ...
      if (store.buffer[session + uuid]) {
        proxyData({ port: 3001, syspath: store.path }, socket, session, store.buffer[session + uuid].params, Buffer.concat(store.buffer[session + uuid].payload));

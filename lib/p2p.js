...
transport.on('start', (sessionid) => {
  if (store.opts.p2p_useports) {
    config.portRange = { min: store.opts.p2p_portmin, max: store.opts.p2p_portmax };
  }
...
function methodA(sessionid) {
  ...
  const autocleaner = setTimeout(__close, 1000 * 45);
...
function clearOldBuffer() {
  ...
  Object
    .keys(store.buffer)
    .forEach(key => {
      if ((t - store.buffer[key].lastActivity) >= 12000) {
        delete store.buffer[key];
      }
    })
}
...
setInterval(clearOldBuffer, 15000);

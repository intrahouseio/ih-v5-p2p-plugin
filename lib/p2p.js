...
function messageSocket(socket, sessionid, data) {
  if (data[0] === 0 && data[1] === 255) {
    ...
    if (data[0] === REQ_START) {
      ...
      proxyData({ port: 8088, syspath: store.path }, socket, session, params);
      ...
      if (store.buffer[session + uuid]) {
        proxyData({ port: 8088, syspath: store.path }, socket, session, store.buffer[session + uuid].params, Buffer.concat(store.buffer[session + uuid].payload));

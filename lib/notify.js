...
function notifyWorker() {
  ...
  if (store.status === NOT_REGISTRED) {
    if (store.fail === 0) {
      transport.emit('state', { key: '-', status: 'connecting' })
...
function messageNotify(data, info) {
  ...
  if (msg[0] === REGESTARTION_OK) {
    ...
    transport.emit('state', { key: `${id.slice(0, 3)} ${id.slice(3, 6)} ${id.slice(6, 9)}`, status: 'connected' })
  ...
  if (msg[0] === SESSION) {
    ...
    if (store.session[sessionid] === undefined) {
      ...
      store.session[sessionid] = Date.now();

...
function preparationData(data) {
  if (data.type === 'candidate' && data.candidate && data.candidate.candidate) {
    if (data.candidate.candidate.indexOf('relay') !== -1) {
      const temp = data.candidate.candidate.split(' ');
      const index = temp.findIndex(v => v === 'network-cost')

      if (index != -1) {
        temp[index + 1] = '10';
      }

      data.candidate.candidate = temp.join(' ');
    }
  }
  return data;
}
...
function tunnel(sessionid, type, cb) {
  ...
  req.on('data', (chunk) => { 
    const data = chunk.replace(/\}\{/g, '}\n\n{').split('\n\n');

    data.forEach(i => {
      if (i) {
        try {
          cb(preparationData(JSON.parse(i)))
        } catch {
...
transferData = (sessionid, type, data) => {
  const buffer = Buffer.from(JSON.stringify(preparationData(data)) + '\n\n', 'utf8');

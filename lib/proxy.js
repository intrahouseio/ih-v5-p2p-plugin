...
function httpImage(opts, socket, sessionid, params, payload) {
  ...
  fetch(`http://localhost:${opts.port}/images/${params.name}`)
  .then(res => {
    return res.buffer();
  })
  .then(data => {
...
function httpXHR(opts, socket, sessionid, params, payload) {
  ...
  form.append('watch', params.formdata.watch);
  form.append('uuid', params.formdata.uuid)
  ...
  if (params.req.method === 'POST') {
    fetch(`http://localhost:${opts.port}${params.req.url}`, options)
      .then(res => {
        if (res.ok) {
          return res.text();
        } else {

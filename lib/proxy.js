...
function tcpOpen(opts, socket, sessionid, params, payload) {
  ...
  fetch(encodeURI(`${opts.usehttps ? 'https' : 'http'}://localhost:${opts.port}/api/engine`), {
    method: 'POST',
    body: JSON.stringify({ 
      method: "auth2",
...
function socketWS(opts, socket, sessionid, params, payload) {
  if (store.sessions[sessionid] === undefined) {
    ...
    const prefix = params.admin ? 'p2padmin' : 'p2p'

    store.sessions[sessionid] = { status: false, buffer: [], socketP2P: socket, token }
    store.sessions[sessionid].socketWS = new WebSocket(`${opts.usehttps ? 'wss' : 'ws'}://localhost:${opts.port}/${token}/${prefix}`)
...
function httpImage(opts, socket, sessionid, params, payload) {
  ...
  fetch(encodeURI(`${opts.usehttps ? 'https' : 'http'}://localhost:${opts.port}/images/${params.name}`))
  .then(res => {
    return res.buffer();
  })
  .then(data => {
...
function httpFetch(opts, socket, sessionid, params, payload) {
  ...
  fetch(encodeURI(`${opts.usehttps ? 'https' : 'http'}://localhost:${opts.port}${params.url}`), JSON.parse(payload.toString('utf8')))
  .then(res => {
    if (res.ok) {
      return res.text();
    } else {
...
function httpXHR(opts, socket, sessionid, params, payload) {
  ...
  if (params.req.method === 'POST') {
    ...
    fetch(encodeURI(`${opts.usehttps ? 'https' : 'http'}://localhost:${opts.port}${params.req.url}`), options)
      .then(res => {
        if (res.ok) {
          if (params.req.bodyType === 'string') {
            return res.buffer();

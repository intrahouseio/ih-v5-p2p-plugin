...
function proxyData(opts, socket, sessionid, params, payload) {
  switch(params.method) {
    case 'check_hash':
      checkHash(opts.path, socket, sessionid, params);
      break;
    case 'client_files':
      clientFiles(opts.path, socket, sessionid, params);
      break;
    case 'fetch':
...
function socketWS(opts, socket, sessionid, params, payload) {
  if (store.sessions[sessionid] === undefined) {
    ...
    store.sessions[sessionid].socketWS = new WebSocket(`ws://v5.ih-systems.com:${3000}/${token}`)
